(function () {
    ...
    function Collection(data) {
        ...
        this.create = function () {
            ...
            this.item.addEventListener('hover:long', function () {
                ...
                items.push({ title: 'Коллeкции @' + data.username, user: data.cid });

                if (voited.indexOf(data.id) == -1) {
                    items = items.concat([
                        { title: '<span class="settings-param__label">+1</span> ' + Lampa.Lang.translate('title_like'), like: 1 },
                        { title: Lampa.Lang.translate('reactions_shit'), like: -1 }
                    ]);
                }
                ...
                Lampa.Select.show({
                    ...
                    onSelect: function onSelect(item) {
                        ...
                        if (item.user) {
                            ...
                            Api.liked({ id: data.id, dir: item.like }, function () {
                                voited.push(data.id);
    ...
    var collections = [
        { hpu: 'new', title: 'Новинки' },
        { hpu: 'top', title: 'В топе' },
        { hpu: 'week', title: 'Популярные за неделю' },
        { hpu: 'month', title: 'Популярные за месяц' },
        { hpu: 'big', title: 'Большие коллекции' },
        { hpu: 'all', title: 'Все коллекции' }
    ];

    function header() {
        var user = Lampa.Storage.get('account', '{}');
        if (!user.token) return false;
        return { headers: { token: user.token, profile: user.id } };
    }
    ...
    function main(params, oncomplite, onerror) {
        ...
        status.onComplite = function () {
            ...
            if (keys.length) {
                var fulldata = [];
                keys.sort(function (a, b) { return sort.indexOf(a) - sort.indexOf(b); });

                keys.forEach(function (key) {
                    var data = status.data[key];
                    data.title = collections.find(function (item) { return item.hpu == key; }).title;
